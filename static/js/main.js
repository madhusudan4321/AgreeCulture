...
function handleImageSelect(event) {
  ...
  reader.onload = (e) => {
    ...
    document.querySelector(".upload-sub").textContent  = `${(file.size / 1024).toFixed(1)} KB`;
...
async function identifyPlant() {
  ...
  try {
    const response = await fetch("/identify-plant", { method: "POST", body: formData });
    const data = await response.json();
...
function displayImageResult(data) {
  ...
  if (!data.success) {
    resultCard.innerHTML = `<div class="card-title">❌ Error</div><p>${data.message || data.error}</p>`;
    return;
  }

  // Plant name and confidence
  document.getElementById("plant-label").textContent = data.plant_label;
  ...
  top3Container.innerHTML = "<h4 style='font-size:0.85rem;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:1px;margin-bottom:0.5rem;'>Top Predictions</h4>";
  ...
  top3.forEach((item, i) => {
    ...
    div.innerHTML = `
      ...
      <div class="top3-crop">${getCropEmoji(item.crop)} ${item.crop.charAt(0).toUpperCase() + item.crop.slice(1)}</div>
      <div class="top3-conf">${item.confidence}%</div>
    `;
